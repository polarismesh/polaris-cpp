	...
	namespace Spec {
		...
		TEST InvalidEscapedCharacters()
		{
			...
			try {
				...
				YAML_ASSERT(e.msg == std::string(YAML_0_3::ErrorMsg::INVALID_ESCAPE) + "c");
				return true;
			}
		...
		TEST InvalidRepeatedYAMLDirective()
		{
			try {
				...
				if(e.msg == YAML_0_3::ErrorMsg::REPEATED_YAML_DIRECTIVE)
					return true;
		...
		TEST CompletelyEmptyNodes()
		{
			...
			YAML_ASSERT(doc[YAML_0_3::Null].to<std::string>() == "bar");
			return true;
		}
		...
		TEST FlowMappingEntries()
		{
			...
			YAML_ASSERT(IsNull(doc[YAML_0_3::Null]));
			return true;
		}
		...
		TEST FlowMappingSeparateValues()
		{
			...
			YAML_ASSERT(doc[YAML_0_3::Null].to<std::string>() == "omitted key");
			return true;
		}
		...
		TEST SinglePairImplicitEntries()
		{
			...
			YAML_ASSERT(doc[1][0][YAML_0_3::Null].to<std::string>() == "empty key entry");
			YAML_ASSERT(doc[2].size() == 1);
		...
		TEST BlockSequenceEntryTypes()
		{
			PARSE(doc, ex8_15);
			YAML_ASSERT(doc.size() == 4);
			YAML_ASSERT(YAML_0_3::IsNull(doc[0]));
