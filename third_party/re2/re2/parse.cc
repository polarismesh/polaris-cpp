...
Regexp* Regexp::RemoveLeadingRegexp(Regexp* re) {
  ...
  if (re->op() == kRegexpConcat && re->nsub() >= 2) {
    ...
    // 3 or more -> 2 or more.
    re->nsub_--;
    memmove(static_cast<void*>(sub), static_cast<const void*>(sub + 1), re->nsub_ * sizeof sub[0]);
    return re;
  }
...
void Regexp::RemoveLeadingString(Regexp* re, int n) {
  ...
  if (re->op() == kRegexpLiteral) {
    ...
    if (n >= re->nrunes_) {
      ...
      re->nrunes_ -= n;
      memmove(static_cast<void*>(re->runes_), static_cast<const void*>(re->runes_ + n),
        re->nrunes_ * sizeof re->runes_[0]);
    }
  }
