...
DFA::State* DFA::CachedState(int* inst, int ninst, uint flag) {
  ...
  memmove(static_cast<void*>(s->inst_), static_cast<const void*>(inst), ninst*sizeof s->inst_[0]);
  s->ninst_ = ninst;
  s->flag_ = flag;
  if (DebugDFA)
    fprintf(stderr, " -> %s\n", DumpState(s).c_str());
...
DFA::StateSaver::StateSaver(DFA* dfa, State* state) {
  ...
  memmove(static_cast<void*>(inst_), static_cast<const void*>(state->inst_), ninst_*sizeof inst_[0]);
}
