 ...
 public:
  ...
  void resize(int new_max_size) {
    ...
    if (new_max_size > max_size_) {
      ...
      if (sparse_to_dense_) {
        memmove(static_cast<void*>(a), static_cast<const void*>(sparse_to_dense_), max_size_*sizeof a[0]);
        if (InitMemory()) {
      ...
      if (dense_) {
        memmove(static_cast<void*>(a), static_cast<const void*>(dense_), size_*sizeof a[0]);
        if (InitMemory()) {
